...
def get_weather(city):
    API_key = "7cc653a955b7575b5271bcbea204b9be"
...
def search():
    ...
    #Get the weather icon image fron the URL and update the icon label 
    image = Image.open(requests.get(icon_url, stream = True).raw)
    icon = ImageTk.PhotoImage(image)
    icon_lable.configure(image = icon)
    icon_lable.image = icon 

    #Update the temperatue and description labels 
...
search_button = ttkbootstrap.Button(root, text = "Search", command = search, bootstyle = "primary-outline")
...
temperature_label = tk.Label(root, font="Helcetica, 20")
